...
<body class="gray-bg">
        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<select class="form-control" name="cid" id = "txt_search_roleid">
						<option value="0">---请选择---</option>
						<c:forEach items="${clist }" var="c">
						 	<option value="${c.cid }">${c.cnmae }</option>
						</c:forEach>
                	</select>
				</div>
			 </div>
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								...
								<td>&nbsp;&nbsp;</td>
								<td>学生生日：</td>
								<td><input type="date" id="edate" name="edate" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>学生性别：</td>
								<td>
									<input type="radio" id="esex" name="esex" value="男" aria-required="true" required/>男
									<input type="radio" id="esex" name="esex" value="女" aria-required="true" required/>女
								</td>
								<td>&nbsp;&nbsp;</td>
								<td>学生爱好：</td>
								<td>
									<input type="checkbox" id="ehobby1" name="ehobby" value="篮球" aria-required="true" required/>篮球
									<input type="checkbox" id="ehobby2" name="ehobby" value="足球" aria-required="true" required/>足球
									<input type="checkbox" id="ehobby3" name="ehobby" value="排球" aria-required="true" required/>排球
								</td>
							</tr>
							<tr>
								<td>学生头像：</td>
								<td>
									<img alt="/pic" src="" id="" width="50px" height="50px">
									<input type="file" id="picFile" name="picFile">
									<input type="hidden" id="pics" name="photo">
								</td>
								<td>&nbsp;&nbsp;</td>
								<td>学生专业：</td>
								<td>
	        ...
	        $('#table_user').bootstrapTable({
	           ...
	           // height: 500,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
	            ...
	            },
	            {
	                ...
	                sortable:true,
	                formatter:function(value,row,index){
	                	return '<img src ="/photo/'+value+'" width="50px" height="50px" >';
	                }
	            },
	            {
	                field: 'ehobby',
	...
	function initEditUser(getSelection){
		...
		$("input[value='"+getSelection.esex+"']").attr("checked",true);
		$('#edate').val(new Date(getSelection.edate).Format("yyyy-MM-dd"));
		$('#ehobby1').attr("checked",getSelection.ehobby.indexOf("篮球")!=-1);
		$('#ehobby2').attr("checked",getSelection.ehobby.indexOf("足球")!=-1);
		$('#ehobby3').attr("checked",getSelection.ehobby.indexOf("排球")!=-1);
		$('#pics').val(getSelection.photo);
	}
