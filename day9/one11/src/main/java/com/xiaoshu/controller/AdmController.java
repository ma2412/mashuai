...
import com.xiaoshu.service.AdmService;
...
@Controller
@RequestMapping("adm")
public class AdmController extends LogController{
	static Logger logger = Logger.getLogger(AdmController.class);
	...
	public String index(HttpServletRequest request,Integer menuid) throws Exception{
		...
		List<Course> clist = admService.findc();
	...
	public void userList(Student student,HttpServletRequest request,HttpServletResponse response,String offset,String limit) throws Exception{
		try {
			...
			PageInfo<Student> userList= admService.findAll(student,pageNum,pageSize,ordername,order);
	...
	public void reserveUser(Student student,HttpServletRequest request,User user,HttpServletResponse response){
		...
		try {
			if (userId != null) {   // userId不为空 说明是修改
				Student userName = admService.existUserWithUserName(student.getName());
					user.setUserid(userId);
					admService.updateUser(student);
					result.put("success", true);
			}else {   // 添加
				if(admService.existUserWithUserName(student.getName())==null){  // 没有重复可以添加
					admService.addUser(student);
					result.put("success", true);
				} else {
					result.put("success", true);
	...
	public void addtemp(Course course,HttpServletRequest request,User user,HttpServletResponse response){
		JSONObject result=new JSONObject();
		
		try {
					admService.addtemp(course);
					result.put("success", true);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("保存用户信息错误",e);
	...
	public void delUser(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			for (String id : ids) {
				admService.deleteUser(Integer.parseInt(id));
			}
			result.put("success", true);
	...
	public void getEcharts(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			List<Student> tlist = admService.getEcharts();
	...
	public void export(Student student,HttpServletRequest request,HttpServletResponse response){
		JSONObject result = new JSONObject();
		try {
			String time = TimeUtil.formatTime(new Date(), "yyyyMMdd");
		    String excelName = "手动备份"+time;
			List<Student> list = admService.findLog(student);
			String[] handers = {"ID","学生编号","学生姓名","年龄","所选专业","所属年级","入校时间"};
			// 1导入硬盘
	...
	private void ExportExcelToDisk(HttpServletRequest request,HttpServletResponse response,
		...
		try {
			...
			for (int i = 0;i < list.size(); i++) {
			   ...
			   // String[] handers = {"学生编号","学生姓名","年龄","所选专业","操作模块","所属年级","入校时间"};
			    ...
			    row.createCell(6).setCellValue(TimeUtil.formatTime(student.getEntrytime(), "yyyy-MM-dd"));
			}
			//写出文件（path为文件路径含文件名）
				OutputStream os;
				File file = new File("f:/aaa"+excleName+".xls");
