...
<body class="gray-bg">
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								...
								<td>&nbsp;&nbsp;</td>
								<td>入校时间：</td>
								<td><input id="entrytime" name="entrytime" class="laydate-icon form-control layer-date" aria-required="true" required/></td>
							</tr>
						</table>
	...
	<!-- 新增课程 -->
	<div class="modal fade" id="modal_user_temp" role="dialog" aria-labelledby="modal_user_temp" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_temp" method="post" action="addtemp.htm">
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_temp_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
				</div>
	...
	<div class="modal fade" id="modal_user_export" role="dialog" aria-labelledby="modal_user_export" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
				<button type="button" class="btn btn-danger"  id="export_user_btnt">导出</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
			</div>
			</div>
		</div>
	...
	$(function () {
	    ...
	    $("#submit_form_temp_btn").click(function(){
	    	$("#form_temp").submit();
	    });
	});
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_temp").click(function(){
	    			$('#modal_user_temp').modal({backdrop: 'static', keyboard: false});
					$('#modal_user_temp').modal('show');
	        });
	    	...
	    	/* $("#btn_echarts").click(function(){
	    		var tb=echarts.init(document.getElementById("main"));
	    		//专业名称数组
	    		var sz=new Array();
	    		//数量数组
	    		var name=new Array();
				...
				$.ajax({
				    ...
				    success:function(res){
						for(var i=0;i<res.length;i++){
							sz[i]={value: res[i].num, name: res[i].cname}
							name[i]=res[i].cname;
						}
						option = {
								 title: {
	    	...
	    	$("#btn_echarts").click(function(){
	    			var echars = echarts.init(document.getElementById("main"));
	    			$.ajax({
	    			    url:"getEcharts.htm",
	    			    dataType:"json",
	    			    type:"post",
	    			    success:function(res){
	    			    	...
	    			    	if(res.success){
	    			    		//循环集合
	    				    	res.tlist.forEach(function(student){
	    				    		xdata.push(student.cname);
	...
	$("#export_user_btnt").click(function(){
		$.ajax({
		    url:"export.htm",
		    dataType:"json",
		    type:"post",
