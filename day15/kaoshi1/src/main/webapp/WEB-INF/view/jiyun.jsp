...
<body class="gray-bg">
			...
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveUser.htm">
						<input type="hidden" name="pId" id="hidden_txt_userid" value="multipart"/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>存款人爱好：</td>
								<td>
									<input type="checkbox" id="pLike1" name="pLike" value="篮球" required/>篮球
									<input type="checkbox" id="pLike2" name="pLike" value="足球" required/>足球
									<input type="checkbox" id="pLike3" name="pLike" value="排球" required/>排球
								</td>
								<td>&nbsp;&nbsp;</td>
								<td>存款人性别：</td>
								<td>
									<input type="radio" id="pSex" name="pSex" value="男" required/>男
									<input type="radio" id="pSex" name="pSex" value="女" required/>女
								</td>
							</tr>
							<tr>
								<td>存款金额：</td>
	...
	<!-- 新增银行 -->
	<div class="modal fade" id="modal_user_addtemp" role="dialog" aria-labelledby="modal_user_addtemp" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_addtemp" method="post" action="addtemp.htm">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>银行名称：</td>
								<td colspan="4"><input type="text" name="bName"
									class="form-control" aria-required="true" required/></td>
							</tr>
						</table>
						
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_addtemp_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	<div class="modal fade" id="modal_user_export" role="dialog" aria-labelledby="modal_user_export" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					 <h4 class="modal-title" id="modal_user_export_head"> 刪除  </h4>
				</div>
				<div class="modal-body">
							导出所有记录？
				</div>
	...
	<div class="modal fade" id="modal_user_echarts" role="dialog" aria-labelledby="modal_user_echarts" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					 <h4 class="modal-title" id="modal_user_echarts_head"> 刪除  </h4>
				</div>
				<div class="modal-body">
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_import" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
							<form action="importJy.htm" method="post" enctype="multipart/form-data" id="form_import">
								<input type="file" name="Excal">
							</form>
				</div>
				<div class="modal-footer">
	...
	$(function () {
	    ...
	    //添加银行
	    var validator = $("#form_addtemp").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
	    ...
	    $("#submit_form_addtemp_btn").click(function(){
	    	$("#form_addtemp").submit();
	    });
	});
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_addtemp").click(function(){
	    		$('#modal_user_addtemp').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_addtemp').modal('show');
	        });
	    	...
	    	$("#btn_echarts").click(function(){
	    			var echatrsjiyun = echarts.init(document.getElementById("main"));
	    			...
	    			$.ajax({
	    			    ...
	    			    success:function(res){
	    			    	...
	    			    	if(res.success){
		    		    		//循环集合
		    			    	res.elist.forEach(function(person){
	...
	function initEditUser(getSelection){
		...
		$('#pLike2').prop("checked",getSelection.pLike.indexOf("足球")!=-1);
		$('#pLike3').prop("checked",getSelection.pLike.indexOf("排球")!=-1);
