...
<body class="gray-bg">
	<div class="panel-body">
        ...
        <div class="row">
			  <div class="col-lg-2">
				...
				<div class="input-group">
			      <span class="input-group-addon">用戶名 </span>
			      <input type="text" name="others" class="form-control" id="txt_search_mmm" >
				</div>
			  </div>
			 <%--  <div class="col-lg-2">
				<div class="input-group">
					<span class="input-group-addon">角色</span>
					<select class="form-control" name="contentcategoryid" id = "txt_search_roleid" aria-required="true" required>
							<option value="">---请选择---</option>
							<c:forEach items="${ylist }" var="y">
							 	<option value="${y.contentcategoryid }">${y.categoryname }</option>
							</c:forEach>
                	</select>
				</div> --%>
			 </div>
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>图片路径：</td>
								<td>
									<img alt="" id="Filepicpath" src="" width="50" height="50">
									<input type="file" id="Filepicpath" name="Filepicpath">
									<input type="hidden" id="picpaths" name="picpath">
									</td>
								<td>&nbsp;&nbsp;</td>
								<td>广告链接：</td>
								<td>
	...
	<div class="modal fade" id="modal_user_echarts" role="dialog" aria-labelledby="modal_user_echarts" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
				<button type="button" class="btn btn-danger"  id="echarts_user_btn">报表</button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
			</div>
			</div>
		</div>
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_echarts").click(function(){	
	    			var echartscontent = echarts.init(document.getElementById("main"))
	    			...
	    			success:function(res){
    			    	...
    			    	var ydata = new Array();
    			    	//循环集合
    			    	res.clist.forEach(function(content){
    			    		xdata.push(content.tname);
    			    		ydata.push({value:content.num,name:content.tname});
    			    	})
    			    	if(res.success){
    			    		option = {
    			    			    xAxis: {
    			    	 ...
    			    	 //将报表数据放入echarts
    			    	 echartscontent.setOption(option);
	    			}
    			});
	...
	function initEditUser(getSelection){
		...
		$('#picpaths').val(getSelection.picpath);
		$('#Filepicpath').attr('src',"/img/"+getSelection.picpath);
	}
